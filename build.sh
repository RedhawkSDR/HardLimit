...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e HardLimit.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/HardLimit-1.0.0
        tar czf ${tmpdir}/HardLimit-1.0.0.tar.gz --exclude=".svn" -C ${tmpdir} HardLimit-1.0.0
        rpmbuild -ta ${tmpdir}/HardLimit-1.0.0.tar.gz
        rm -rf $tmpdir
    else
